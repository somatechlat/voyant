...
def record_activity_failure(
    activity_name: str, error_type: str, duration_seconds: Optional[float] = None
):
    """
    ...
    try:
        ...
        if duration_seconds is not None:
            ACTIVITY_DURATION.labels(activity_name=activity_name).observe(
                duration_seconds
            )
...
def record_circuit_breaker_triggered(activity_name: str, service_name: str):
    ...
    try:
        CIRCUIT_BREAKER_TRIGGERED.labels(
            activity_name=activity_name, service_name=service_name
        ).inc()
        logger.info(
            f"Circuit breaker blocked {activity_name} (service: {service_name})"
        )
    except Exception as e:
    ...
    try:
        ...
        if success:
            ...
            WORKFLOW_FAILED.labels(
                workflow_name=workflow_name, error_type=error_type or "Unknown"
            ).inc()
...
def track_workflow_execution(workflow_name: str):
    ...
    try:
        ...
        error_type = type(e).__name__
        record_workflow_completion(
            workflow_name, duration, step_count, success=False, error_type=error_type
        )
        raise
