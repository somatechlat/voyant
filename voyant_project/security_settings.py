...
class SecuritySettings(BaseSettings):
    ...
    jwt_issuer: str = Field(
        default="voyant-api",
        description="JWT token issuer (iss claim)."
    )
    ...
    security_enabled: bool = Field(
        default=True,
        description="Enable security headers and protections."
    )
    ...
    content_security_policy: str = Field(
        default="default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'",
    ...
    database_ssl_key: Optional[str] = Field(
        default=None,
        description="Path to SSL key for database connections."
    )
    ...
    audit_log_level: str = Field(
        default="INFO",
        description="Audit log level (DEBUG, INFO, WARNING, ERROR)."
    )
    audit_log_file: Optional[str] = Field(
        default="/var/log/voyant/audit.log",
    ...
    audit_log_include_requests: bool = Field(
        default=True,
        description="Include HTTP requests in audit logs."
    )
    audit_log_include_responses: bool = Field(
        default=True,
        description="Include HTTP responses in audit logs."
    )
    ...
    security_monitoring_threshold_failed_logins: int = Field(
        default=5,
        description="Threshold for failed login alerts."
    )
    security_monitoring_threshold_suspicious_requests: int = Field(
        default=100,
        description="Threshold for suspicious request alerts."
    )
