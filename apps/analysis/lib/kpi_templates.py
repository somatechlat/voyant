...
_register(
    ...
    sql="""
        WITH customer_rfm AS (
            SELECT
                {customer_id_column} as customer_id,
                date_diff('day', MAX({date_column}), CURRENT_DATE) as recency_days,
...
_register(
    ...
    sql="""
        SELECT
            ...
            AVG(date_diff('day', {order_date_column}, {delivery_date_column})) as avg_lead_days,
            MIN(date_diff('day', {order_date_column}, {delivery_date_column})) as min_lead_days,
            MAX(date_diff('day', {order_date_column}, {delivery_date_column})) as max_lead_days
        FROM {table}
...
def get_template(name: str) -> KPITemplate | None:
