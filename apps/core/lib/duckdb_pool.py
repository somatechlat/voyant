...
class DuckDBConnectionPool:
    ...
    def initialize(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.info(
                    f"Pool initialized with {self.config.min_connections} connections"
                )
    ...
    def return_connection(self, conn: duckdb.DuckDBPyConnection):
        ...
        try:
            ...
            logger.debug(
                f"Returned connection to pool (checkins: {self._total_checkins})"
            )
...
def initialize_pool(config: Optional[PoolConfig] = None):
    ...
    if config is None:
        ...
        config = PoolConfig(
            database_path=settings.duckdb_path, min_connections=2, max_connections=10
        )
