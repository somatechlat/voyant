...
def _require_context_for_policy(context: SomaContext) -> None:
    ...
    if not context.session_id or not context.user_id:
        raise SomaContextError(
            "Missing X-Soma-Session-ID or X-User-ID header in context. Policy cannot be enforced."
        )
...
async def enforce_policy(
    ...
    if not settings.soma_policy_url:
        logger.debug(
            "Soma Policy Engine URL not configured. Skipping policy enforcement."
        )
        return None
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            raise SomaPolicyUnavailable(
                "Policy engine unavailable or returned an error."
            ) from exc
...
async def remember_summary(
    ...
    if not context.session_id:
        logger.warning(
            "Skipping Soma memory write: missing X-Soma-Session-ID in context."
        )
        return
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            raise SomaIntegrationError(
                "Failed to write to Soma Memory Gateway."
            ) from exc
...
async def create_task_for_job(
    ...
    if not tenant_uuid or not user_uuid:
        logger.warning(
            "Skipping Soma task creation: Tenant ID or User ID is not a valid UUID format."
        )
        return None
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            raise SomaIntegrationError(
                "Failed to create task in Soma Orchestrator."
            ) from exc
...
async def update_task_status(
    ...
    if not settings.soma_orchestrator_url:
        logger.debug(
            "Soma Orchestrator URL not configured. Skipping task status update."
        )
        return
    ...
    if not tenant_uuid:
        logger.warning(
            "Skipping Soma task update: Tenant ID is not a valid UUID format."
        )
        return
    ...
    if context.user_id and _parse_uuid(context.user_id):
        params["actor_principal_id"] = (
            context.user_id
        )  # If user ID is a valid UUID, include as actor.
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            raise SomaIntegrationError(
                "Failed to update task status in Soma Orchestrator."
            ) from exc
