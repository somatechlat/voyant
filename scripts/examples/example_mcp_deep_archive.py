...
async def main():
    ...
    payload = {
        ...
        "download_patterns": ["descargar", "archivo", "bajar", ".pdf"],
    ...
    print(
        f"Final DOM State captured: {len(result.get('interaction_states', {}))} states."
    )
    print(
        f"Files Downloaded via Generic Pattern Matching: {len(result.get('files_downloaded', []))}"
    )
