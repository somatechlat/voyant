    ...
    def _get_jwks(self) -> Dict[str, Any]:
        ...
        if self._jwks is None:
            try:
                ...
                raise HttpError(503, "Authentication service (Keycloak) unavailable.") from exc
        return self._jwks
    ...
    def validate_token(self, token: str) -> User:
        ...
        try:
            ...
            if not key:
                ...
                raise HttpError(401, "Invalid authentication token: signing key not found.")
            ...
            raise HttpError(500, "Authentication failed due to internal error.") from exc
...
def require_role(required_role: str):
    ...
    def role_checker(request) -> User:
        ...
        if not user.has_role(required_role):
            logger.warning(f"User {user.username} (tenant: {user.tenant_id}) attempted to access resource requiring role '{required_role}' without permission.")
...
def require_permission(required_permission: str):
    ...
    def permission_checker(request) -> User:
        ...
        if not user.has_permission(required_permission):
            logger.warning(f"User {user.username} (tenant: {user.tenant_id}) attempted to access resource requiring permission '{required_permission}' without permission.")
            raise HttpError(403, f"Access denied: Permission '{required_permission}' required.")
        return user
