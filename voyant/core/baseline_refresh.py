...
class RefreshStatus:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "next_scheduled": (
                datetime.fromtimestamp(self.next_scheduled).isoformat()
                if self.next_scheduled
                else None
            ),
            "is_due": self.is_due,
            "hours_until_next": (
                max(0, (self.next_scheduled - time.time()) / 3600)
                if self.next_scheduled
                else None
            ),
...
class BaselineRefreshManager:
    ...
    def set_policy(self, source_id: str, policy: RefreshPolicy) -> None:
        ...
        logger.info(
            f"Set refresh policy for {source_id}: every {policy.interval_hours}h"
        )
    ...
    async def start_scheduler(self, check_interval_seconds: int = 300) -> None:
        ...
        logger.info(
            f"Started baseline refresh scheduler (check every {check_interval_seconds}s)"
        )
