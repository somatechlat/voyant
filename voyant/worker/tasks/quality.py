...
def run_quality(
    ...
    try:
        ...
        self.update_state(
            state="RUNNING", meta={"progress": 10, "status": "Loading data"}
        )
        ...
        self.update_state(
            state="RUNNING", meta={"progress": 20, "status": "Building expectations"}
        )
        ...
        for i, check in enumerate(checks_to_run):
            ...
            self.update_state(
                state="RUNNING",
                meta={"progress": progress, "status": f"Running {check}"},
            )
        ...
        self.update_state(
            state="RUNNING", meta={"progress": 85, "status": "Calculating score"}
        )
        ...
        self.update_state(
            state="RUNNING", meta={"progress": 90, "status": "Saving results"}
        )
        ...
        if overall_score < 0.8:
            logger.warning(
                f"Quality score {overall_score:.2f} below threshold for {source_id}"
            )
