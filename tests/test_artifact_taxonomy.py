...
class TestArtifactTypeIdentification:
    """
    Tests the `get_artifact_type` function to ensure it correctly returns
    the human-readable description for canonical artifact keys.
    """

    def test_get_profile_type(self):
        """
        Verifies that `get_artifact_type` correctly identifies "profile.json"
        as a "Profile artifact".
        """
        assert get_artifact_type("profile.json") == "Profile artifact"

    def test_get_quality_type(self):
        """
        Verifies that `get_artifact_type` correctly identifies "quality.html"
        as a "Quality artifact".
        """
        assert get_artifact_type("quality.html") == "Quality artifact"

    def test_get_chart_type(self):
        """
        Verifies that `get_artifact_type` correctly identifies a chart key
        as a "Chart artifact".
        """
    ...
    def test_get_sufficiency_type(self):
        """
        Verifies that `get_artifact_type` correctly identifies "sufficiency.json"
        as a "Sufficiency score".
        """
        assert get_artifact_type("sufficiency.json") == "Sufficiency score"

    def test_get_unknown_type(self):
        """
        Ensures that `get_artifact_type` returns "Unknown" for unrecognized keys.
        """
...
class TestGeneratorArtifactCompliance:
    """
    Tests to ensure that the artifact keys expected from core and extended
    generators comply with the defined taxonomy.
    """

    def test_core_generators_produce_valid_keys(self):
        """
        Verifies that a sample set of keys from core generators are valid.
        """
        core_artifacts = [
            "profile.html",
            "profile.json",
    ...
    def test_extended_generators_produce_valid_keys(self):
        """
        Verifies that a sample set of keys from extended generators are valid.
        """
        extended_artifacts = [
            "quality.html",
            "quality.json",
    ...
    def test_manifest_always_valid(self):
        """
        Ensures that the artifact key for a job manifest is always valid.
        """
        assert is_valid_artifact_key("manifest.json")
...
class TestArtifactKeyNormalization:
    """
    Tests for helper functions that normalize or extract components from artifact keys.
    """

    def test_extract_base_name(self):
        """
        Verifies that the base name (without extension or path) is correctly extracted.
        """
    ...
    def test_extract_format(self):
        """
        Verifies that the file format (extension) is correctly extracted from the artifact key.
        """
