...
class SearchClient:
    ...
    def search_apis(self, query: str, limit: int = 10) -> List[Dict[str, str]]:
        ...
        if not self.base_url:
            logger.warning(
                "SERPER_SEARCH_URL not set. Returning empty search results for query: '%s'.",
                query,
            )
            return []
        ...
        if not self.api_key:
            logger.warning(
                "SERPER_API_KEY not set. Returning empty search results for query: '%s'.",
                query,
            )
        ...
        try:
            ...
            logger.warning(
                "Serper API circuit breaker is open. Returning empty results for query: '%s'.",
                query,
            )
