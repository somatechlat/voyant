...
def tool_connect(name: str, source_type: str, connection_config, credentials=None, sync_schedule=None, tenant_id=None):
...
def tool_ingest(source_id: str, mode: str = "full", tables=None, tenant_id=None):
    ...
    _start_workflow(
        IngestDataWorkflow,
        f"ingest-{job.job_id}",
        IngestParams(job_id=str(job.job_id), source_id=source_id, mode=mode, tables=tables).__dict__,
    )
    job.status = "running"
...
def tool_analyze(source_id=None, table=None, tables=None, sample_size: int = 10000, kpis=None, analyzers=None, tenant_id=None):
...
def tool_artifact(artifact_id: str, tenant_id=None):
    artifact = Artifact.objects.filter(artifact_id=artifact_id, tenant_id=_tenant(tenant_id)).first()
    if not artifact:
...
def tool_search(query: str, limit: int = 5, tenant_id=None):
    ...
    results = store.search(query_vector=vec, k=limit, filter_metadata={"tenant_id": _tenant(tenant_id)})
...
def tool_lineage(urn: str, direction: str = "both", depth: int = 3):
    ...
    for d in directions:
        resp = httpx.post(
            ...
            json={"query": query, "variables": {"urn": urn, "direction": d, "depth": min(depth, 10)}},
            timeout=30.0,
        )
        ...
        for rel in data:
            ...
            nodes.add(eurn)
            edges.append({"source": eurn if d == "UPSTREAM" else urn, "target": urn if d == "UPSTREAM" else eurn, "type": rel.get("type", "PRODUCES")})
...
def tool_sources_list(tenant_id=None):
    ...
    return [{"source_id": str(s.id), "name": s.name, "source_type": s.source_type, "status": s.status} for s in sources]
...
def tool_jobs_list(tenant_id=None, status=None, job_type=None, limit: int = 50):
    ...
    return [{"job_id": str(j.job_id), "job_type": j.job_type, "status": j.status, "progress": j.progress} for j in q]
...
def tool_artifacts_list(job_id: str, tenant_id=None):
    rows = Artifact.objects.filter(job_id=job_id, tenant_id=_tenant(tenant_id)).order_by("-created_at")
    return [
        {
...
def tool_governance_schema(urn: str):
    response = httpx.get(f"{settings.datahub_gms_url}/aspects/{urn}?aspect=schemaMetadata", timeout=30.0)
...
def tool_quotas_tiers():
    ...
    return [{"tier": t.value, "description": manager.policies[t].description} for t in QuotaTier if t in manager.policies]
...
def tool_presets_get(job_id: str):
    ...
    return {"job_id": str(p.job_id), "preset_name": p.preset_name, "status": p.status, "parameters": p.parameters}
...
def tool_discovery_services_register(name: str, base_url: str, spec_url=None, version: str = "1.0.0", owner: str = "unknown", tags=None):
    parser = SpecParser()
    service = ServiceDef(name=name, base_url=base_url, version=version, metadata={"owner": owner, "tags": ",".join(tags or [])})
    if spec_url:
...
def tool_vector_index(text: str, metadata=None, item_id=None, tenant_id=None):
    ...
    store.save()
    return {"id": final_id, "status": "indexed", "dimensions": embedding_result.dimensions}


@mcp_app.tool(name="scrape.fetch")
def tool_scrape_fetch(url: str, engine: str = "playwright", wait_for=None, scroll: bool = False, timeout: int = 30, wait_until=None, settle_ms=None, block_resources=None, capture_json: bool = False, capture_url_contains=None, capture_max_bytes=None, capture_max_items=None):
    return run_async(
...
def tool_scrape_ocr(images, language: str = "spa+eng"):
    return run_async(ScrapeActivities().process_ocr, {"images": images, "language": language})


@mcp_app.tool(name="scrape.parse_pdf")
def tool_scrape_parse_pdf(pdf_url: str, extract_tables: bool = False):
    return run_async(ScrapeActivities().parse_pdf, {"pdf_url": pdf_url, "extract_tables": extract_tables})


@mcp_app.tool(name="scrape.transcribe")
def tool_scrape_transcribe(media_urls, language: str = "es"):
    return run_async(ScrapeActivities().transcribe_media, {"media_urls": media_urls, "language": language})
