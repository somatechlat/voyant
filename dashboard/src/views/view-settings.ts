                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            <div class="flex gap-2 mt-2">
                                ...
                                <button class="px-3 py-1.5 text-sm font-semibold bg-brand text-white rounded-lg hover:bg-black" @click=${() => this.save(s.key)}>Save</button>
                                <button class="px-3 py-1.5 text-sm border border-gray-200 rounded-lg hover:bg-gray-50" @click=${() => { this.editing = ''; }}>Cancel</button>
                            </div>` : html`
                            <span class="text-sm font-mono ${s.is_secret ? 'text-gray-300' : 'text-gray-700'}">${s.value}</span>`}
                        </div>
                        ${this.editing !== s.key && !s.is_secret ? html`
                        <button class="text-xs text-brand font-semibold hover:underline ml-4" @click=${() => { this.editing = s.key; this.editValue = s.value; }}>Edit</button>` : ''}
                    </div>
                </div>`)}
