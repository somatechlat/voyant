...
class LagoClient:
    ...
    async def emit_usage(
        ...
        try:
            ...
            metric_code = self.METRICS.get(
                metric, metric
            )  # Use mapped code or raw metric name.
            ...
            logger.debug(
                f"Emitted usage: {customer_id}/{metric}={value} (Lago code: {metric_code})."
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to emit usage event to Lago for customer {customer_id}, metric {metric}: {e}"
            )
            return False
    ...
    async def create_subscription(
        ...
        try:
            ...
            data = response.json()
            logger.info(
                f"Created subscription in Lago: {customer_id} subscribed to {plan_code}."
            )
            return data.get("subscription")

        except Exception as e:
            logger.error(
                f"Failed to create subscription for customer '{customer_id}' and plan '{plan_code}': {e}"
            )
            return None
    ...
    async def get_current_usage(self, customer_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to retrieve current usage for customer '{customer_id}' from Lago: {e}"
            )
            return None
