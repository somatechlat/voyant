...
def _detect_source_type(hint: str) -> Dict[str, Any]:
    ...
    if hint_lower.startswith("postgresql://") or hint_lower.startswith("postgres://"):
        return {"source_type": "postgresql", "connector": "airbyte/source-postgres", "properties": {"host": hint.split("@")[-1].split("/")[0] if "@" in hint else "unknown"}, "confidence": 0.95}
    ...
    if "s3://" in hint_lower:
        return {"source_type": "s3", "connector": "airbyte/source-s3", "properties": {"bucket": hint.split("/")[2] if len(hint.split("/")) > 2 else ""}, "confidence": 0.9}
