...
class ColumnProfile:
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {
            ...
            "missing_rate": (
                round(self.missing_count / self.count, 4) if self.count > 0 else 0
            ),
        }
            ...
            result.update(
                {
                    ...
                    "median": (
                        round(self.median, 4) if self.median is not None else None
                    ),
                    "std_dev": (
                    ...
                    "skewness": (
                        round(self.skewness, 4) if self.skewness is not None else None
                    ),
                }
            )
            ...
            result.update(
                {
                    ...
                    "top_values": [
                        {"value": v, "count": c} for v, c in (self.top_values or [])
                    ],
                }
            )
...
class SegmentComparison:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "mean_difference": (
                round(self.mean_diff, 4) if self.mean_diff is not None else None
            ),
            "mean_diff_percent": (
                round(self.mean_diff_percent, 2)
                if self.mean_diff_percent is not None
                else None
            ),
            "std_difference": (
                round(self.std_diff, 4) if self.std_diff is not None else None
            ),
            "distribution_overlap": (
                round(self.distribution_overlap, 4)
                if self.distribution_overlap is not None
                else None
            ),
...
def calculate_distribution_overlap(
    ...
    db = 0.25 * math.log(0.25 * (var1 / var2 + var2 / var1 + 2)) + 0.25 * (
        (mean1 - mean2) ** 2
    ) / (var1 + var2)
...
def compare_segments(
    ...
    common_columns = set(profile_a.column_profiles.keys()) & set(
        profile_b.column_profiles.keys()
    )
    ...
    for col in common_columns:
        ...
        if col_a.std_dev and col_b.std_dev:
            overlap = calculate_distribution_overlap(
                col_a.mean, col_a.std_dev, col_b.mean, col_b.std_dev
            )
...
def generate_segment_report(
    profiles: Dict[str, SegmentProfile], compare_all: bool = True
) -> Dict[str, Any]:
    """
