...
class TestClusterData:
    ...
    @pytest.mark.asyncio
    async def test_cluster_basic(self, activities, env):
    ...
    @pytest.mark.asyncio
    async def test_cluster_empty_data_raises(self, activities, env):
        """Empty data raises non-retryable ApplicationError."""
        with pytest.raises(ApplicationError, match="No data provided"):
            await env.run(activities.cluster_data, {"data": []})

    @pytest.mark.asyncio
    async def test_cluster_missing_data_raises(self, activities, env):
        """Missing data key raises non-retryable ApplicationError."""
        with pytest.raises(ApplicationError, match="No data provided"):
            await env.run(activities.cluster_data, {})

    @pytest.mark.asyncio
    async def test_cluster_default_clusters(self, activities, env):
        """Default cluster count of 3 is used when not specified."""
        data = [{"x": float(i), "y": float(i * 2)} for i in range(30)]
        result = await env.run(activities.cluster_data, {"data": data})
        assert result is not None

    @pytest.mark.asyncio
    async def test_cluster_two_clusters(self, activities, env):
    ...
    @pytest.mark.asyncio
    async def test_cluster_returns_silhouette(self, activities, env):
...
class TestTrainClassifierModel:
    ...
    @pytest.mark.asyncio
    async def test_classifier_basic(self, activities, env):
    ...
    @pytest.mark.asyncio
    async def test_classifier_empty_data_raises(self, activities, env):
        """Empty data raises non-retryable ApplicationError."""
        with pytest.raises(ApplicationError, match="No data provided"):
            await env.run(activities.train_classifier_model, {"data": []})

    @pytest.mark.asyncio
    async def test_classifier_missing_data_raises(self, activities, env):
        """Missing data key raises non-retryable ApplicationError."""
        with pytest.raises(ApplicationError, match="No data provided"):
            await env.run(activities.train_classifier_model, {})

    @pytest.mark.asyncio
    async def test_classifier_default_target(self, activities, env):
        """Default target column 'target' is used when not specified."""
        data = [
            {"feature1": float(i), "target": i % 2} for i in range(20)
        ]
        result = await env.run(activities.train_classifier_model, {"data": data})
        assert result is not None
...
class TestTrainRegressionModel:
    ...
    @pytest.mark.asyncio
    async def test_regression_basic(self, activities, env):
    ...
    @pytest.mark.asyncio
    async def test_regression_empty_data_raises(self, activities, env):
        """Empty data raises non-retryable ApplicationError."""
        with pytest.raises(ApplicationError, match="No data provided"):
            await env.run(activities.train_regression_model, {"data": []})

    @pytest.mark.asyncio
    async def test_regression_missing_data_raises(self, activities, env):
        """Missing data key raises non-retryable ApplicationError."""
        with pytest.raises(ApplicationError, match="No data provided"):
            await env.run(activities.train_regression_model, {})

    @pytest.mark.asyncio
    async def test_regression_perfect_correlation(self, activities, env):
...
class TestForecastTimeSeries:
    ...
    @pytest.mark.asyncio
    async def test_forecast_empty_dates_raises(self, activities, env):
    ...
    @pytest.mark.asyncio
    async def test_forecast_empty_values_raises(self, activities, env):
    ...
    @pytest.mark.asyncio
    async def test_forecast_missing_both_raises(self, activities, env):
        """Missing both dates and values raises non-retryable ApplicationError."""
        with pytest.raises(ApplicationError, match="Dates and values are required"):
            await env.run(activities.forecast_time_series, {})

    @pytest.mark.asyncio
    async def test_forecast_with_prophet(self, activities, env):
