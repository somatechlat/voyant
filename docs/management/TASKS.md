...
## 18. External Agent Context (Decommissioned SomaAgentHub Integration)

> **Status: DECOMMISSIONED** — The SomaAgentHub integration (`apps/integrations/soma.py`) was
> hard deleted per executive directive. All session context is now passed via standard HTTP headers
> (`X-Tenant-ID`, `X-Request-ID`, `traceparent`) processed by existing middleware.
> Voyant operates as a standalone MCP-native platform. No further Soma-specific integration is planned.

Definition of Done: N/A — decommissioned.
