...
def execute_preset(
    ...
    try:
        ...
        if "profile" in preset.get("output_artifacts", []):
            self.update_state(
                state="RUNNING", meta={"progress": 20, "status": "Running profiling"}
            )
            ...
            results["sub_jobs"].append(
                {"type": "profile", "task_id": profile_result.id}
            )

        if "quality" in preset.get("output_artifacts", []):
            self.update_state(
                state="RUNNING",
                meta={"progress": 40, "status": "Running quality checks"},
            )
            ...
            results["sub_jobs"].append(
                {"type": "quality", "task_id": quality_result.id}
            )

        if "kpi" in preset.get("output_artifacts", []):
            self.update_state(
                state="RUNNING", meta={"progress": 60, "status": "Computing KPIs"}
            )
            # Execute KPI SQL
        ...
        if "chart" in preset.get("output_artifacts", []):
            self.update_state(
                state="RUNNING", meta={"progress": 80, "status": "Generating charts"}
            )
        ...
        self.update_state(
            state="RUNNING", meta={"progress": 90, "status": "Aggregating results"}
        )
