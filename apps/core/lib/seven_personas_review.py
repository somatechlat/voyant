...
class FullReviewReport:
    ...
    def summary(self) -> str:
        ...
        for review in self.reviews:
            status = (
                "✅" if review.score >= 0.8 else "⚠️" if review.score >= 0.6 else "❌"
            )
            lines.append(
                f"{status} {review.persona}: {review.score:.1%} ({review.passed_checks}/{review.total_checks})"
            )
        ...
        # Critical findings
        critical = [
            f for r in self.reviews for f in r.findings if f.severity == "critical"
        ]
        if critical:
