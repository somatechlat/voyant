...
class Settings(BaseSettings):
    ...
    trino_schema: str = Field(
        default="", description="Default Trino schema to query."
    )
    ...
    def check_security(self) -> "Settings":
        ...
        if self.env != "local":
            ...
            if not str(self.minio_secret_key).strip() or not str(self.keycloak_client_secret).strip():
                import logging
...
def get_settings() -> Settings:
    ...
    try:
        from voyant.core.system_settings import get_db_settings_overrides

        overrides = get_db_settings_overrides(
            settings=settings,
            runtime_keys=Settings.RUNTIME_ENV_KEYS,
            secret_keys=Settings.SECRET_KEYS,
        )
        if overrides:
            settings = settings.model_copy(update=overrides)
    except Exception:
        # Never break startup because DB settings are unavailable.
        pass
