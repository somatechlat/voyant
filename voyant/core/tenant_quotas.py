...
class QuotaManager:
    ...
    def check_quota(
        self, tenant_id: str, resource: ResourceType, requested_amount: float = 1.0
    ) -> QuotaCheckResult:
        """
    ...
    def _get_usage_in_period(
        self, tenant_id: str, resource: ResourceType, period_seconds: int
    ) -> float:
    ...
    def _get_current_concurrent(self, tenant_id: str, resource: ResourceType) -> float:
    ...
    def cleanup_old_records(self, max_age_seconds: int = 86400 * 7):
        ...
        with self._lock:
            for tenant_id in self._usage:
                ...
                self._usage[tenant_id] = [
                    r for r in self._usage[tenant_id] if r.timestamp >= cutoff
                ]
